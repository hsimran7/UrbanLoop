    ...
    <div className="space-y-8 pb-24 text-slate-800">
      ...
      <div className="glass-card p-8 flex justify-between items-center">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight font-display">Waste Traceability & Custody Audit</h1>
          <p className="text-sm text-slate-600 mt-2 font-medium">Verify material lineage from bin origin points, through transit weighbridge logs, up to facility processing.</p>
        </div>
      </div>
      ...
      {!isLoading && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[
            { label: 'Total Waste Batches', val: loads.length, col: 'text-sky-700' },
            { label: 'In Transit', val: loads.filter(l => l.status === 'IN_TRANSIT').length, col: 'text-amber-700' },
            { label: 'Delivered / Arrived', val: loads.filter(l => ['ARRIVED', 'WEIGHED', 'ACCEPTED', 'PARTIALLY_REJECTED', 'REJECTED', 'CLOSED'].includes(l.status)).length, col: 'text-teal-700' },
            { label: 'Closed Cycles', val: loads.filter(l => l.status === 'CLOSED').length, col: 'text-emerald-700' },
          ].map((s) => (
            <div key={s.label} className="glass-card p-5 text-center">
              <div className={`text-3xl font-black ${s.col}`}>{s.val}</div>
              <div className="text-[10px] text-slate-500 mt-1.5 uppercase font-extrabold tracking-wider">{s.label}</div>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="space-y-1.5">
            <label className="text-xs text-slate-500 font-extrabold uppercase tracking-wider block">Search batch loads</label>
            <input
              type="text"
              value={searchQuery}
          ...
          <h3 className="text-xs font-extrabold text-slate-500 uppercase tracking-wider px-1">Waste Load Batches</h3>

          {isLoading ? (
            <div className="glass-card p-12 text-center">
              <div className="h-8 w-8 border-2 border-nature-accent border-t-transparent rounded-full animate-spin mx-auto mb-3" />
              <p className="text-slate-500 text-xs font-medium">Loading custody feeds...</p>
            </div>
          ) : filteredLoads.length === 0 ? (
            <p className="text-slate-500 text-xs py-4 text-center font-medium">No matching loads found.</p>
          ) : (
            <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
              {filteredLoads.map((l) => (
                <div
                  ...
                  className={`glass-card p-4 transition cursor-pointer flex flex-col gap-2 hover:shadow-md ${
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase ${
                      l.status === 'CLOSED' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' :
                      l.status === 'IN_TRANSIT' ? 'bg-sky-50 text-sky-700 border border-sky-200' :
                  ...
                  <div className="flex justify-between text-[11px] text-slate-500 font-medium">
                    <span>Category: <strong className="text-slate-700">{l.wasteType}</strong></span>
                    <span>Items: <strong className="text-slate-700">{l.items?.length || 0} bins</strong></span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <h3 className="text-xs font-extrabold text-slate-500 uppercase tracking-wider px-1">End-to-End Lineage Flow</h3>

          {loadingTrace ? (
            <div className="glass-card p-16 text-center">
              <div className="h-8 w-8 border-2 border-nature-accent border-t-transparent rounded-full animate-spin mx-auto mb-3" />
              <p className="text-slate-500 text-xs font-medium">Tracing waste lifecycle pipeline...</p>
            </div>
          ) : !traceDetail ? (
            <div className="glass-card p-16 text-center">
              <div className="text-4xl mb-3">🕸️</div>
              <p className="text-slate-800 text-sm font-bold">Select a load batch from the list to view trace pipeline.</p>
              <p className="text-slate-500 text-xs mt-1 font-medium">Audit complete custody trails from origin properties up to recycling outputs.</p>
            </div>
          ) : (
            <div className="space-y-6">
              {/* Batch General info */}
              <div className="glass-card p-5 grid grid-cols-3 gap-4 text-xs font-medium">
                <div>
                  <span className="text-slate-500 block">Load Batch</span>
                  <strong className="text-slate-800 text-sm font-extrabold">{traceDetail.loadCode}</strong>
                </div>
                <div>
                  <span className="text-slate-500 block">Waste Type</span>
                  <strong className="text-slate-800 text-sm font-extrabold">{traceDetail.wasteType}</strong>
                </div>
                <div>
