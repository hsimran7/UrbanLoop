...
export default function AdminSchedulesPage() {
  ...
  return (
    <div className="space-y-8 pb-24 text-slate-800">
      {/* Title */}
      <div className="glass-card p-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">Schedule Management</h1>
          <p className="text-sm text-slate-600 mt-2 font-medium">Configure weekly recurring waste services and register schedule exception overrides</p>
        </div>
        
        {/* Area Selector Filter */}
        <div className="flex items-center space-x-3 shrink-0">
          <span className="text-xs text-slate-600 font-extrabold">Active Area Grid:</span>
          {areas.length > 0 ? (
            <select
            ...
            <span className="text-xs text-slate-400">Loading areas...</span>
          )}
        </div>
      </div>
