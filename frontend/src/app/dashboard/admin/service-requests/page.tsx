  ...
  return (
    <div className="space-y-8 pb-24 text-slate-100">
      ...
      <div className="p-6 rounded-2xl border border-slate-800 bg-gradient-to-br from-slate-900/80 to-indigo-950/20 backdrop-blur">
        <div className="text-xs text-indigo-400 font-semibold uppercase tracking-widest mb-1">Operations Control</div>
        <h1 className="text-2xl font-bold text-slate-100 font-display">Municipal Support Board</h1>
        <p className="text-sm text-slate-400 mt-1">Triage, allocate, resolve service requests, and manage SLA escalations.</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
        {[
          { label: 'Unassigned Requests', val: requests.filter(r => !r.assignedDepartmentId).length, color: 'text-amber-400' },
          { label: 'Open Tickets', val: requests.filter(r => r.status !== 'CLOSED' && r.status !== 'RESOLVED' && r.status !== 'CANCELLED' && r.status !== 'REJECTED').length, color: 'text-indigo-400' },
          { label: 'SLA Warnings / At Risk', val: requests.filter(r => r.sla?.acknowledgmentBreached || r.sla?.resolutionBreached).length, color: 'text-red-400' },
          { label: 'Escalated Breaches', val: requests.filter(r => r.sla?.resolutionBreached).length, color: 'text-rose-500' },
          { label: 'Reopened Complaints', val: requests.filter(r => r.status === 'REOPENED').length, color: 'text-yellow-400' },
          { label: 'Resolved (Awaiting Close)', val: requests.filter(r => r.status === 'RESOLVED').length, color: 'text-emerald-450' },
        ].map((s) => (
          <div key={s.label} className="p-4 rounded-xl border border-slate-800 bg-slate-900/40">
            <div className={`text-2xl font-bold ${s.color}`}>{s.val}</div>
            <div className="text-[9px] text-slate-500 mt-1 uppercase font-semibold tracking-wider">{s.label}</div>
          </div>
        ))}
      </div>

      {/* Queue filter controls */}
      <div className="p-5 border border-slate-850 bg-slate-900/10 rounded-2xl flex flex-wrap gap-4 items-end text-xs">
        <div className="space-y-1.5 flex-1 min-w-[200px]">
          <label className="text-slate-400 font-medium block">Search query</label>
          <input
            type="text"
            value={searchQuery}
        ...
        <div className="space-y-1.5 min-w-[120px]">
          <label className="text-slate-400 font-medium block">Priority</label>
          <select
